...
export default function BoardingPassLesson11() {
  const [menuOpen, setMenuOpen] = useState(false);
  const { isLightMode, toggleTheme } = useTheme();

  useEffect(() => {
    document.title = 'Understanding Wallets | Contraband';
  }, []);
...
.progress-bar { height: 100%; background: #F7931A; width: 52.38%; transition: width 0.3s ease; }
...
.mobile-menu-btn { display: none; flex-direction: column; justify-content: center; align-items: center; width: 44px; height: 44px; background: transparent; border: none; cursor: pointer; padding: 0; z-index: 1001; }
.mobile-menu-btn span { display: block; width: 24px; height: 2px; background: #e8e4dc; transition: all 0.3s ease; margin: 3px 0; }
.lesson-page.light-mode .mobile-menu-btn span { background: #0a0a0a; }
.mobile-menu-btn.open span:nth-child(1) { transform: rotate(45deg) translate(5px, 5px); }
.mobile-menu-btn.open span:nth-child(2) { opacity: 0; }
.mobile-menu-btn.open span:nth-child(3) { transform: rotate(-45deg) translate(6px, -6px); }
.mobile-menu-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: #0a0a0a; z-index: 999; display: flex; flex-direction: column; justify-content: center; align-items: center; opacity: 0; visibility: hidden; transition: opacity 0.3s ease, visibility 0.3s ease; }
.lesson-page.light-mode .mobile-menu-overlay { background: #e8e4dc; }
.mobile-menu-overlay.open { opacity: 1; visibility: visible; }
.mobile-menu-nav { display: flex; flex-direction: column; align-items: center; gap: 2rem; }
.mobile-menu-nav a { font-family: 'Cormorant Garamond', serif; font-size: 2rem; color: #e8e4dc; text-decoration: none; text-transform: uppercase; letter-spacing: 0.1em; transition: color 0.3s ease; }
.lesson-page.light-mode .mobile-menu-nav a { color: #0a0a0a; }
.mobile-menu-nav a:active { color: #F7931A; }
.mobile-menu-nav a.coming-soon { text-decoration: line-through; opacity: 0.5; }
      ...
      <div className={`lesson-page ${isLightMode ? 'light-mode' : ''}`}>
        ...
        <header className="lesson-header">
          <div className="lesson-breadcrumb">
            ...
            <span className="breadcrumb-sep">→</span>
            <span className="breadcrumb-current">Lesson 11</span>
          </div>
          <div className="lesson-meta">
            <span className="lesson-number">Lesson 11 of 21</span>
            <span className="lesson-week">Week 2</span>
            <span className="lesson-duration">16 min read</span>
          </div>
          <h1 className="lesson-title">Understanding Wallets</h1>
          <p className="lesson-subtitle">Hot, cold, custodial, non-custodial—explained.</p>
        </header>
        ...
        <nav className="lesson-navigation">
          <Link href="/learn/boarding-pass/your-first-purchase" className="nav-btn">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
            Lesson 10
          </Link>
          <Link href="/learn/boarding-pass/your-first-wallet" className="nav-btn primary">
            12
